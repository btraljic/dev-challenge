// todo: (10) 2. would be cool if we actually queried the graphql server for companies and displayed them here.
        ...
        <Query query={query}>
          {({ loading, error, data }) => {
            ...
            return (
              <FlatList
                data={data.companies}
                keyExtractor={item => item.id /* ***** riješen missing keys warning */}
                renderItem={({ item }) => (
                  <TouchableOpacity
                    onPress={() =>
